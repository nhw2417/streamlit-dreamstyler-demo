...
@click.option("--num_samples", default=6)
@click.option("--seed")
def style_transfer(
    sd_path=None,
    ...
    half = len(outputs) // 2
    top_row = np.concatenate([np.asarray(img) for img in outputs[:half]], axis=1)
    bottom_row = np.concatenate([np.asarray(img) for img in outputs[half:]], axis=1)
    outputs = np.concatenate([top_row, bottom_row], axis=0)
